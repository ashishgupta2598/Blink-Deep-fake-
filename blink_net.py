...
class BlinkCNN(object):
    ...
    def loss(self):
        ...
        self.net_loss = tf.reduce_mean(input_tensor=self.net_loss)
        tf.compat.v1.losses.add_loss(self.net_loss)
        # L2 weight regularize
        self.L2_loss = tf.reduce_mean(input_tensor=[self.cfg.TRAIN.BETA * tf.nn.l2_loss(v)
                     for v in tf.compat.v1.trainable_variables() if 'weights' in v.name])
...
class BlinkLRCN(object):
    ...
    def build(self):
        self.input = tf.compat.v1.placeholder(dtype=tf.float32,
                                    shape=[self.batch_size, self.max_time, self.img_size[0], self.img_size[1], self.img_size[2]])
        self.blined_gt = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size])
        self.eye_state_gt = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size, self.max_time])
        self.seq_len = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size])
    ...
    def _vgg16(self, input):
        ...
        if self.is_train:
            layers.fc6 = tf.nn.dropout(layers.fc6, rate=1 - (0.5))
    ...
    def _rnn_cell(self, input):
        with tf.compat.v1.variable_scope('rnn_cell'):
            ...
            if self.rnn_type == 'LSTM':
                cell = tf.compat.v1.nn.rnn_cell.LSTMCell(self.hidden_unit)
            elif self.rnn_type == 'GRU':
                cell = tf.compat.v1.nn.rnn_cell.GRUCell(self.hidden_unit)
            else:
    ...
    def _avg_rnn_out(self, rnn_out):
        ...
        avg = tf.reduce_sum(input_tensor=rnn_out, axis=1) / tf.expand_dims(seq_len, axis=-1)
        return avg
    ...
    def loss(self):
        ...
        for batch_id in range(self.batch_size):
            ...
            loss_per_batch = tf.reduce_sum(input_tensor=tf.gather(loss_per_batch, tf_idx, axis=0)) / seq_len
            self.net_loss.append(loss_per_batch)
        self.net_loss = tf.reduce_mean(input_tensor=self.net_loss)
        tf.compat.v1.losses.add_loss(self.net_loss)
        # L2 weight regularize
        self.L2_loss = tf.reduce_mean(input_tensor=[self.cfg.TRAIN.BETA * tf.nn.l2_loss(v)
                                       for v in tf.compat.v1.trainable_variables() if 'weights' in v.name or 'kernel' in v.name])
        tf.compat.v1.losses.add_loss(self.L2_loss)
        self.total_loss = tf.compat.v1.losses.get_total_loss()