...
def conv2D(input,
           ...
           padding='SAME',
           strides=(1, 1),
           weights_initializer=tf.initializers.GlorotUniform(),#xavier_initializer(),
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        out = tf.nn.conv2d(input=input, filters=kernel, strides=strides, padding=padding)
...
def max_pool(input,
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        strides = [1, strides[0], strides[1], 1]
        out = tf.nn.max_pool2d(input=input, ksize=ksize, strides=strides, padding=padding)
...
def avg_pool(input,
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ksize = [1, ksize[0], ksize[1], 1]
        strides = [1, strides[0], strides[1], 1]
        out = tf.nn.avg_pool2d(input=input, ksize=ksize, strides=strides, padding=padding)
...
def batch_norm(input, name, is_train=True, params={}):
    ...
    #     print i.name
    var_list = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope=name)  # beta, gamma, moving_mean, moving_variance
